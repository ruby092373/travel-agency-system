...
*(建議您在專案中建立一個 `images` 資料夾，並放入系統截圖，然後在此處引用)*

*   **顧客登入與註冊**
    *(在此處放入截圖，例如： `![Login](images/login.png)`)*
*   **顧客選購與日曆操作**
    *(在此處放入截圖)*
*   **員工後台訂單管理**
    *(在此處放入截圖)*
...
透過這個專案，我深入理解了 MVC 架構在實際應用中的優勢，並掌握了 JDBC 與連線池的進階用法。解決 `JDatePicker` 的事件衝突與處理 `LEFT JOIN` 的空值問題，大幅提升了我的除錯能力。

**未來預計加入的功能：**
*   匯出訂單報表 (PDF/Excel)。
*   圖表化顯示銷售統計 (使用 JFreeChart)。
*   顧客密碼重置功能。
